...
namespace atFrameWork2.SeleniumFramework
{
    class WebItem
    {
        ...
        public bool isDisplayedElementIn(string xp, IWebDriver driver = default)
        {
            bool isDisp = false;
            Execute((el,drv) =>
            {
                if (drv.FindElements(By.XPath(xp)).Count == 0)
                    isDisp = false;
                else isDisp = true;
        ...
        public int CountOfTheElement(string xp, IWebDriver driver = default)
        {
            var count = 0;
            Execute((el, drv) =>
            {
               count = drv.FindElements(By.XPath(xp)).Count;
